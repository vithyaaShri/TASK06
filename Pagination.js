...
/*Variable to Store Start and End Values of Index and Size of Table*/
var array=[];
var array_length=0;
var table_size=8;
var start_index=1;
...
/*Calcuate the max index to display in a page*/
function preloadCalculations(){
    array=myarr;
    array_length=array.length;
    max_index=array_length/table_size;
    /*If we get remainder add 1 or else take the max_index as such*/
    if((array_length % table_size)>0){
        max_index++;
    }
}
/*Used to display button from 1 to end and also the prev and next button and the function to perform on click*/
function displayIndexButtons(){
    preloadCalculations();
    $(".index_button  button").remove()
    $(".index_button").append('<button onclick="prev();">Previous</button>')

        for(var i=1;i<=max_index;i++){
            $(".index_button").append('<button onclick="indexpagination('+i+');" index="'+i+'">'+i+'</button>')
        }
    $(".index_button").append('<button onclick="next();">Next</button>')
    highlightIndexButton()

}


/*Highlight the current button active*/
...
/*Display the start and end of table row based on page number*/
function displayTableRows(){
    $(".table-responsive table tbody tr").remove();
    var table_start=start_index -1;
    var table_end=end_index;
...
displayIndexButtons();
/*Function to perform on Clicking next Button*/
function next(){
...
/*Function to perform on Clicking previous Button*/
function prev(){
  if(current_index>1){
    current_index--;
    highlightIndexButton();
  ...
  /*Function to perform on Clicking Number Button*/
